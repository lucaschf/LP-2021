...
void Program::generateReport(Stock stock, ProductType type)
{
	...
	cout << spaces << "Product \t Quantity \t Unitary price \t Total value" << endl;
	for (int i = 0; i < stock.getProductCount(); i++) {
		auto p = stock.getProduct(i);

		cout << spaces << p.getName() << " \t " << p.getQuantity() << " \t "
			<< p.getPrice() << " \t R$ " << p.getPriceForQuantity() << endl;
	}

	cout << spaces << "Total of items in stock: \t" << stock.getProductCount()
		<< "\t Stock total value: \t R$ " << stock.getTotalValue() << endl << endl;
}
